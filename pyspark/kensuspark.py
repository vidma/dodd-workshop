...
def patched_spark_stop(wrapped, dam_wait_timeout_secs):
    def wrapper(self):
        try:
            print('DAM: in spark.stop, waiting for DAM reporting to finish')
            wait_for_dam_completion(self, dam_wait_timeout_secs)
            print('DAM: in spark.stop, waiting for DAM done, stopping spark context')
        except:
            import traceback
            print("unexpected DAM issue: {}".format(traceback.format_exc()))
        result = wrapped(self)
        print('DAM: in spark.stop, spark context stopped')
        return result
...
def patched_spark_createDataFrame_pandas(wrapped, pandas_to_spark_df_via_tmp_file, tmp_dir=None):
    def wrapper(self, data, *args, **kwargs):
        try:
            print('Kensu: before calling spark.createDataFrame')
            from kensu import pandas as ksu_pandas
            if isinstance(data, ksu_pandas.data_frame.DataFrame):
                print('Kensu: replacing kensu.pandas.data_frame.DataFrame wrapper into a original pandas...')
                if not pandas_to_spark_df_via_tmp_file:
                    ...
                    print('Kensu: spark.createDataFrame(pandas_df): Kensu will write pandas_df to a temp file - '+ abs_tmp_file_name)
                    ...
                    print('Kensu: spark.createDataFrame(pandas_df): Kensu will read to a temp file in spark - ' + spark_temp_file_path)
                    spark_df = self.read.parquet(spark_temp_file_path)
                    return spark_df
            print('Kensu: done modifying arguments to spark.createDataFrame')
        except:
            import traceback
            print("unexpected DAM issue: {}".format(traceback.format_exc()))
        result = wrapped(self, data, *args, **kwargs)
        print('Kensu: end of spark.createDataFrame')
        return result
...
def patched_dataframe_write(wrapped):
    def wrapper(self):
        try:
            print('DAM: in patched DataFrame.write, marking DataFrame as cached')
            from pyspark.sql import DataFrameWriter
            result = DataFrameWriter(self.cache())
            print('DAM: in patched DataFrame.write, returning result')
        except:
            import traceback
            print("DAM: unexpected issue in DataFrame.write: {}".format(traceback.format_exc()))
            from pyspark.sql import DataFrameWriter
            result = DataFrameWriter(self.cache())
        return result
...
def patched_dataframe_toPandas(wrapped):
    def wrapper(self):
        try:
            print('DAM: in patched DataFrame.toPandas()')
            result = wrapped(self)
            print('DAM: in patched DataFrame.toPandas(), returning result')
        except:
            import traceback
            print("DAM: unexpected issue in DataFrame.toPandas(): {}".format(traceback.format_exc()))
            result = wrapped(self)
        return result
...
def do_disable_spark_writes(spark):
    ...
    print('DAM disabling df.write done')
...
def set_fake_timestamp(spark, mocked_timestamp):
    ...
    """
    print('DAM set_fake_timestamp starting')
    sc = spark.sparkContext
    jvm = sc._jvm
    ...
    print('DAM set_fake_timestamp done')
...
def add_ds_path_sanitizer(spark, search_str, replacement_str):
    ...
    Sets a fake timestamp to be reported to DAM
    """
    print('DAM add_ds_path_sanitizer starting')
    sc = spark.sparkContext
    jvm = sc._jvm
    ...
    print('DAM add_ds_path_sanitizer done')
...
def report_df_as_dam_datasource():
    def report_as_dam_datasource(self, # type: DataFrame
        ...
        try:
            print('DAM: in df.report_as_dam_datasource')
            call_df_as_dam_datasource(self, location, datasource_type or 'in-memory-dataframe', name, logical_name)
            print('DAM: df.report_as_dam_datasource started in background')
        except:
            import traceback
            print("unexpected DAM issue: {}".format(traceback.format_exc()))
        return self
...
def report_df_as_dam_jdbc_datasource():
    def report_as_dam_jdbc_datasource(self,  # type: DataFrame
        ...
        try:
            print('DAM: in df.report_as_dam_jdbc_datasource')
            call_df_as_dam_jdbc_datasource(self, db_type, schema_name, table_name, maybe_db_name, name, logical_name)
            print('DAM: df.report_as_dam_jdbc_datasource started in background')
        except:
            import traceback
            print("unexpected DAM issue: {}".format(traceback.format_exc()))
        return self
...
def report_df_as_kpi():
    def report_as_kpi(self,  # type: DataFrame
        ...
        try:
            ...
            print('DAM: df.report_as_kpi started in background')
        except:
            import traceback
            print("unexpected DAM issue: {}".format(traceback.format_exc()))
        return self
...
def patch_dam_df_helpers():
    try:
        print('Adding DataFrame.report_as_dam_datasource')
        from pyspark.sql import DataFrame
        DataFrame.report_as_dam_datasource = report_df_as_dam_datasource()
        print('done adding DataFrame.report_as_dam_datasource')
    except:
        import traceback
        print("unexpected issue when patching DataFrame.report_as_dam_datasource: {}".format(traceback.format_exc()))
    try:
        print('Adding DataFrame.report_as_dam_jdbc_datasource')
        from pyspark.sql import DataFrame
        DataFrame.report_as_dam_jdbc_datasource = report_df_as_dam_jdbc_datasource()
        print('done adding DataFrame.report_as_dam_jdbc_datasource')
    except:
        import traceback
        print("unexpected issue when patching DataFrame.report_as_dam_jdbc_datasource: {}".format(traceback.format_exc()))
    try:
        print('Adding DataFrame.report_as_kpi')
        from pyspark.sql import DataFrame
        DataFrame.report_as_kpi = report_df_as_kpi()
        print('done adding DataFrame.report_as_kpi')
    except:
        import traceback
        print("unexpected issue when patching DataFrame.report_as_kpi: {}".format(traceback.format_exc()))
...
def get_inputs_lineage_fn(dam, df):
    ...
    # print('lineage_from_scala:', lineage_from_scala)
    ...
    print('lineage_info:', lineage_info)
...
def init_kensu_DAM(
        ...
        offline_mode_file_name='dam-offline.log',
        ...
        allow_invalid_ssl_certificates=False,
        stats=True,
        input_stats=False,
        input_stats_for_only_used_columns=True,
        input_stats_keep_filters=True,
        input_stats_compute_quantiles=False,
        input_stats_cache_bypath=True,
        input_stats_coalesce_enabled=True,
        input_stats_coalesce_workers=1,
        output_stats_compute_quantiles=False,
        output_stats_cache_bypath=False,
        output_stats_coalesce_enabled=False,
        output_stats_coalesce_workers=100,
        cache_output_for_datastats=False,
        use_short_datasource_names=True,
        dam_shutdown_timeout_secs=10 * 60,
        dam_debug_file_level="INFO",
        dam_debug_file_enabled=False,
        ...
        h2o=False,
        add_dam_dataframe_helpers=True,
        disable_spark_writes=False,
        environment=None,
        fake_timestamp=None,
        projects=None,  # type: list[str]
        project=None,
        h2o_create_virtual_training_datasource=True,
        pandas_conversions=True,
        pandas_to_spark_df_via_tmp_file=True,
        pandas_to_spark_tmp_dir='/tmp/spark-to-pandas-tmp'
):
    if is_dam_tracking_enabled:
        print("Initializing DAM...")
        ...
        try:
            ...
            if dam_debug_file_enabled:
                ...
                print("Will write dam DAM log to file:" + dam_debug_filename)
            ...
            if (dam_shutdown_timeout_secs is not None) and (dam_shutdown_timeout_secs > 0):
                print('patching spark.stop to wait for DAM reporting to finish')
                from pyspark.sql import SparkSession
                SparkSession.stop = patched_spark_stop(SparkSession.stop, dam_shutdown_timeout_secs)
                print('patching spark.stop done')
            ...
            if disable_spark_writes:
                try:
                    ...
                    import traceback
                    print("unexpected issue when disabling df.write {}".format(traceback.format_exc()))

            if cache_output_for_datastats and stats:
                try:
                    print('patching DataFrame.write to cache/persist the results to speedup data-stats computation')
                    from pyspark.sql import DataFrame
                    DataFrame.write = patched_dataframe_write(DataFrame.write)
                    print('done patching DataFrame.write')
                except:
                    import traceback
                    print("unexpected issue when patching DataFrame.write: {}".format(traceback.format_exc()))
            ...
            if pandas_conversions:
                try:
                    print('patching DataFrame.toPandas')
                    from pyspark.sql import DataFrame
                    ...
                    print('done patching DataFrame.toPandas')
                except:
                    import traceback
                    print("unexpected issue when patching DataFrame.toPandas: {}".format(traceback.format_exc()))
                try:
                    print('patching spark.createDataFrame to work with pandas dataframes')
                    from pyspark.sql import SparkSession
                    ...
                    print('patching  spark.createDataFrame done')
                except:
                    import traceback
                    print("unexpected issue when patching DataFrame.toPandas: {}".format(traceback.format_exc()))
                try:
                    print('adding spark env var spark.executorEnv.KSU_DISABLE_PY_COLLECTOR=true'
                    ...
                    print('done adding spark.executorEnv.KSU_DISABLE_PY_COLLECTOR')
                except:
                    import traceback
                    print("unexpected issue when adding spark.executorEnv.KSU_DISABLE_PY_COLLECTOR: {}".format(traceback.format_exc()))
            ...
            print("Error when initializing DAM tracker:")
            import traceback
